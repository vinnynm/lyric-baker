 ...
 *
 * Fixes:
 *  - rebuildIlst had a dead first filter followed by the real filter; the OR
 *    logic in the first filter was wrong (an atom could slip through if typeBytes
 *    matched but type string had an encoding edge-case). Replaced with a single
 *    clean filter on the type string only.
 */
internal object ItunesAtomWriter {
    ...
    // The ©lyr type string as Kotlin sees it (0xA9 = © in Latin-1)
    private val LYRICS_TYPE = String(LYRICS_ATOM, Charsets.ISO_8859_1)
    ...
    private fun injectLyrics(
        src: ByteArray, moovAtom: Atom, lyrics: LyricDocument, options: M4aBakeOptions
    ): ByteArray {
    ...
    private fun rebuildUdta(src: ByteArray, udta: Atom, lyrics: LyricDocument, opts: M4aBakeOptions): ByteArray {
        ...
        if (children.none { it.type == META }) newChildren += buildMetaFromScratch(lyrics, opts)
    ...
    private fun rebuildMeta(src: ByteArray, meta: Atom, lyrics: LyricDocument, opts: M4aBakeOptions): ByteArray {
        ...
        if (children.none { it.type == ILST }) newChildren += buildIlstWithLyrics(lyrics, opts)
    ...
    private fun rebuildIlst(src: ByteArray, ilst: Atom, lyrics: LyricDocument, opts: M4aBakeOptions): ByteArray {
        ...
        // FIX: original had two filters — a broken OR-logic one followed by the real one.
        // Single filter on type string is unambiguous.
        val newChildren = children
            .filter { it.type != LYRICS_TYPE }
            .map { src.copyOfRange(it.offset, it.offset + it.totalSize) }
            .toMutableList()

        if (opts.writeLyrics) newChildren += buildLyricsAtom(lyrics)
    ...
    private fun buildLyricsAtom(lyrics: LyricDocument): ByteArray {
        ...
        val dataHeader  = byteArrayOf(0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00)
        ...
        for (atom in atoms) {
            when (atom.type) {
                ...
                MOOV, TRAK, MDIA, MINF, STBL ->
                    patchOffsets(data, atom.dataOffset, atom.dataOffset + atom.dataSize, delta)
            }
        }
    }
